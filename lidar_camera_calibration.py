...
class Form(QObject):
    ...
    def init_subscriber(self):
        ...
        self.sub_camera = rospy.Subscriber("/raw_image", Image, self.camera_msg_callback)
        self.sub_lidar = rospy.Subscriber("/merge_points", PointCloud2, self.lidar_msg_callback)
    ...
    def draw_frames(self):
        ...
        if self.lidar_msg is not None:
            ...
            self.lidar_map = cv2.line(self.lidar_map, (0, int((self.lidar_x - self.line_x)*self.scale)), (int(self.w), int((self.lidar_x - self.line_x)*self.scale)), (255, 0, 0), 2)
            self.lidar_map = cv2.line(self.lidar_map, (int((self.lidar_y - self.line_y)*self.scale), 0), (int((self.lidar_y - self.line_y)*self.scale), self.h), (0, 255, 0), 2)
